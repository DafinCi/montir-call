...
export default function JobDetailSheet({ job, isOpen, onClose, onAccept }) {
  if (!job) return null;
      ...
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto p-0 flex flex-col justify-between">
        <div className="p-6 space-y-6">
          ...
          <SheetHeader className="text-left space-y-2">
            ...
            <SheetTitle className="text-xl font-bold">{job.customerName}</SheetTitle>
            ...
            <div className="p-3.5 rounded-xl border bg-muted/20 space-y-2">
              <div className="flex items-start gap-2.5">
                ...
                <div className="text-xs space-y-0.5">
                  <p className="font-semibold text-card-foreground">{job.locationTitle}</p>
              ...
              <div className="flex items-center justify-between text-xs pt-2 border-t border-border/50 text-muted-foreground">
                ...
                <span className="font-semibold text-card-foreground">{job.distanceKm} KM ({job.estimatedTime})</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="p-3 rounded-lg border bg-card">
                <span className="text-muted-foreground block text-[10px]">Tipe Kendaraan</span>
                <span className="font-semibold text-card-foreground">{job.vehicleModel}</span>
              </div>
              <div className="p-3 rounded-lg border bg-card">
                <span className="text-muted-foreground block text-[10px]">Plat Nomor</span>
                <span className="font-mono font-semibold text-card-foreground">{job.licensePlate}</span>
              </div>
            </div>
          </div>
          ...
          {job.customerNote && (
            <div className="p-3 rounded-lg border border-amber-500/20 bg-amber-500/5 text-amber-900 dark:text-amber-200 text-xs flex items-start gap-2">
              ...
              <span><strong>Catatan Pelanggan:</strong> {job.customerNote}</span>
            </div>
          )}
